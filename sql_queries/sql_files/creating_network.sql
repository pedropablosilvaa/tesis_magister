...
CREATE TABLE capas_estaticas.roads_routing as (
	...
	source, 
	target, 
	ST_Length(ST_Transform(geom, 32718)) as cost, 
	ST_Transform(geom, 32718) as geom  
	FROM capas_estaticas.roads_final
);

-- se elimina la capa de isocronas
	...
	JOIN (
		...
		FROM pgr_drivingdistance(
			'SELECT 
				...
				target, 
				cost 
			from capas_estaticas.roads_routing', 
			ARRAY[724,721], 150, false)) 
AS route 
