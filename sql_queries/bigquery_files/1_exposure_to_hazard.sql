...
DROP TABLE IF EXISTS `tesispregrado.tesis_magister.exposure_to_hazard_01`;

-- se crea la capa de ruteo
CREATE TABLE `tesispregrado.tesis_magister.exposure_to_hazard_01` as 
...
count_ccaa_by_entidad as (
    SELECT
        entidad.id,
        ST_AsText(entidad.geom) AS geom_str,
    ...
    LEFT JOIN 
        ccaa_estado as ccaa
    ON 
        ST_DWithin(entidad.geom,  ccaa.geom, 5000)
    GROUP BY
        entidad.id, geom_str
),
...
SELECT
    ...
    ST_AsText(calculate_dist_hazard.geom) as geom
